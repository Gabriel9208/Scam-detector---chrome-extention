...
def scraper(url:str):
    ...
    try:
        ...
        footers = driver.find_elements(By.XPATH, '//footer//*[text()]') +\
            driver.find_elements(By.XPATH, "//*[contains(@class, 'footer')]//*[text()]")
        
        #footer_list = []
        for footer_elements in footers:
            if footer_elements:                
                    ...
                    for category, keywords in keys.items():
                        for elmt in footer_elements:
                            ...
                            for key in keywords:
                                if key in elmt.text:
                                    find = True
                                    retv = extract_info(category, key, elmt)
                                    if retv:
                                        info[category] = extract_info(category, key, elmt)
                                    break
                            if find: break
                    ...
                    for category, keywords in keys.items():
                        for key in keywords:
                            if key in footer_elements.text:
                                find = True
                                retv = extract_info(category, key, footer_elements)
                                if retv:
                                    info[category] = extract_info(category, key, footer_elements)
                                break
            else:
                print("Footer is empty.")
                return None
...
if __name__ == "__main__":
    #matchList = scraper("https://www.bnext.com.tw/") # 47sec
    matchList = scraper("https://www.momoshop.com.tw/main/Main.jsp") # 1min
